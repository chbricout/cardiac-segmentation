...
def runTraining():
    ...
    CE_loss = torch.nn.CrossEntropyLoss(weight=torch.FloatTensor([ 0.03, 0.4, 0.4, 0.17]))
    ...
    for i in range(epoch):
        ...
        for j, data in enumerate(train_loader_full):
            ...
            lossTotal = CE_loss_value  + lambda_supervised_prop * prop_diff_value + lambda_c1_dice*c1_dice_loss
            ...
            printProgressBar(
                ...
                length=15,
                suffix=" Loss: {:.4f}, prop diff : {:.4f}, dice: {:.4f}, gan : {:.4f}".format(
                    lossTotal, prop_diff_value,c1_dice_loss, loss_bce_fake + loss_bce_real
                ),
            )
        ...
        if i >= 0:
            ...
            for j, data in enumerate(train_loader_unlabeled):
                ...
                # net_predictions = softMax(raw_predictions)
                ceil_pred_for_disc=  torch.nn.functional.softmax(raw_predictions.detach()/0.0001)
                ceil_pred_for_prop=  torch.nn.functional.softmax(raw_predictions/0.0001)
                ...
                lossTotal =   torch.pow((prop_by_layer(ceil_pred_for_prop) - stat_prior),2).mean()
                ...
                if i >= 10:
                    ...
                    likely_loss = discNet(sum_pred)
                    lossTotal += lambda_likely* likely_loss.mean()
